...
async function getRawSortedPosts() {
  ...
  const sorted = allBlogPosts.sort(
    (
      a: { data: { sticky: number; published: string | number | Date } },
      b: { data: { sticky: number; published: string | number | Date } },
    ) => {
      const stickyA = a.data.sticky || 0;
      const stickyB = b.data.sticky || 0;
      // Sticky posts first (higher sticky value first)
      if (stickyA !== stickyB) {
        return stickyB - stickyA;
      }
      // Then sort by published date (newer first)
...
export async function getCategoryList(): Promise<Category[]> {
  ...
  allBlogPosts.forEach((post: { data: { category: string[] } }) => {
    ...
    post.data.category.forEach((cat: string) => {
      ...
      count[catName] = count[catName] ? count[catName] + 1 : 1;
    });
  });
